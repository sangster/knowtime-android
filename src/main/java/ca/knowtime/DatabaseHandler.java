        ...
        extends SQLiteOpenHelper
{
    ...
    private static final String KEY_SHORTNAME = "m_shortName";
    ...
    public Stop getStop( String code ) {
        ...
        Cursor cursor = db.query( TABLE_STOP,
                                  new String[]{ KEY_CODE, KEY_NAME, KEY_LATITUDE, KEY_LONGITUDE, KEY_FAVOURITE },
        ...
        if( cursor.getCount() > 0 ) {
            ...
            stop = new Stop( cursor.getString( 0 ), cursor.getString( 1 ), Double.parseDouble( cursor.getString( 2 ) ),
                             Double.parseDouble( cursor.getString( 3 ) ), isFavourite );
        }
        return stop;
    }
    ...
    public Route getRoute( String shortName ) {
        SQLiteDatabase db = this.getReadableDatabase();
        Route route = null;
        Cursor cursor = db.query( TABLE_ROUTE, new String[]{ KEY_SHORTNAME, KEY_LONGNAME, KEY_ID, KEY_FAVOURITE },
                                  KEY_SHORTNAME + "= ?", new String[]{ shortName }, null, null, null, null );
        if( cursor.getCount() > 0 ) {
            cursor.moveToFirst();
            boolean isFavourite = false;
